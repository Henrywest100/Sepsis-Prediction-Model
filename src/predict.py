...
class SepsisPredictor:
    def __init__(self, model_path: Union[str, Path, None] = None):
        ...
        self.feature_names = payload["features_names"]
        self.medians = payload["medians"]
        self.scaler = payload.get("scaler")  # ✅ ADDED: Load the scaler
    ...
    def predict_patient(self, patient_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        row["gender_Other/Not specified"] = 0  
        ...
        if self.scaler is not None:
            feat_array_scaled = self.scaler.transform(feat_array)
        else:
            feat_array_scaled = feat_array

        # Model Predict
        risk_score = float(self.model.predict_proba(feat_array_scaled)[0, 1])
