...
try:
    ...
    from feature_engineering import build_feature
except ImportError:
...
def train_and_evaluate_models(
    ...
    processed_csv_path = data_dir / "sepsis_feature.csv"
    ...
    # Check if any column is perfectly correlated with y
    correlations = df_features.corr()['sepsis_event'].abs().sort_values(ascending=False)
    print(f"\nTop correlations with sepsis_event:")
    ...
    baseline_lr = LogisticRegression(max_iter=100, class_weight='balanced', random_state=30)
    ...
    primary_hgb.fit(X_train_scaled, y_train)
    #primary_hgb.fit(X_train, y_train) 

    hgb_probs = primary_hgb.predict_proba(X_test_scaled)[:,1]
    ...
    print("\n============== Primary Model (HistGradient Boosting Classifier) =============")
    ...
    metadata = {
        ...
        'primary_metrics':{
            ...
            'accuracy': round(hgb_acc),
            'f1_score': round(hgb_f1, 4)

        },
        'baseline_metrics': {
            'model_type': 'LogisticRegession',
